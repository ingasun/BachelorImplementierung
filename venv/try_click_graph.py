...
def hilighter(event):
    ...
    for node in event.nodes:
        ...
        # hier in dem dict sind nur die colors
        # print(graph.nodes[node][label_intermediate[key])]
    ...
    # den return wert kann man wahrscheinlich nirgendwo verwenden, aber vielleicht in datenbank speichern
    # return event.nodes
...
intermediate_states = [(('s', 't', 'u'), {"name": 'bla'}), (('t', 'v'), {'color': 'green'}), ('s', 't', 'v'), ('t', 'u'), ('t',), ('s', 't', 'u', 'v'), ('t', 'u', 'v'), ('s', 'u', 'v'), ('s', 't'), ('u', 'v'), ('s', 't', 'u'), ('t', 'u'), ('s', 't', 'u', 'v'), ('u',), ('s', 'u'), ('s', 'u', 'v'), ('t', 'u', 'v'), ('u', 'v')]
...
node_list_with_attributes = [(('s', 't', 'u'), {"color": "red"}), (('t', 'v'), {'color': 'green'}), ('s', 't', 'v'), ('t', 'u'), ('t',), ('s', 't', 'u', 'v'), ('t', 'u', 'v'), ('s', 'u', 'v'), ('s', 't'), ('u', 'v'), ('s', 't', 'u'), ('t', 'u'), ('s', 't', 'u', 'v'), ('u',), ('s', 'u'), ('s', 'u', 'v'), ('t', 'u', 'v'), ('u', 'v')]
nodes = state_list + intermediate_states
merged_dict = {**label_node_state, **label_intermediate}
G = nx.DiGraph()  # or DiGraph, MultiGraph, MultiDiGraph, etc

# so könnte man node attributes hinzufügen:
...
# nx.set_node_attributes(G, {('s', 't'): 't'}, 'label')
# raise text positions
# pos_nodes = nx.spring_layout(G)
# nx.draw_networkx_labels(G, pos=pos, labels=merged_dict, font_size=7, font_family='sans-serif')
# hier wird oder wurde die position der label verschoben
#pos_attrs = {}
#for node, coords in pos.items():
#    pos_attrs[node] = (coords[0], coords[1] - 2)
# nx.draw_networkx_labels(G, pos=pos, labels=merged_dict, font_size=7, font_family='sans-serif')
# nx.draw_networkx(G, with_label=True, node_color='green', pos=nx.spring_layout(G))
# nx.set_node_attributes(G, label_intermediate, "labels")
# K3 Graph zeichnen
